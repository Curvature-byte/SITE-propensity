"""PyTorch re-implementation of the SITE (Similarity preserved Individual Treatment Effect) network.

This module mirrors the original TensorFlow graph defined in ``simi_ite/site_net.py``.
It provides:
    * ``SITEConfig`` – hyper-parameters matching the TensorFlow ``FLAGS`` and ``dims``.
    * ``SITENetwork`` – representation encoder, TARNet/S-learner head, and PDDM branch.
    * ``compute_site_loss`` – faithful loss composition (factual, weight decay, mid-point,
      and PDDM penalties) including sample re-weighting by the average treatment rate.

The implementation integrates with ``pytorch_implementation.tep_propensity_pddm`` for
three-pair selection and similarity targets. Dropout and batch norm behavior follow the
original defaults (keep probability in TF becomes ``dropout`` probability here).
"""
