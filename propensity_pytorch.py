"""PyTorch re-implementation of ``simi_ite/propensity.py``.

Features:
- Drop-in training function that mirrors ``propensity_score_training`` but relies on
  PyTorch modules (logistic regression or a small MLP) instead of scikit estimators.
- Handles train/validation split, class imbalance, and accuracy/F1 reporting.
- Returns an artifact object with ``predict_proba`` helper plus ``save``/``load`` utilities.
"""
